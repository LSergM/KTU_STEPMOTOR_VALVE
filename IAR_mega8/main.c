...
void main( void )
{
  unsigned char din=0,pwm=0, sp=0, delstart = 0;  
...
//                      while(1);
                  ...
                  else position = position + D_pc.ModBUS_rw_reg[SIZE_STEP];
                }                
        }
      }            
    }
...
void InitCPU(void)
{
    ...
    D_pc.ModBUS_r_reg[DRV_STATE] &= ~0x01FF;
      D_pc.ModBUS_r_reg[DRV_STATE] |= (1 << DRV_DIR); //Движение в начальную позицию 
    update_drv();
    steps_drv = 511;
    position = 0xFFFF;    
}
